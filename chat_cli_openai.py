#!/usr/bin/env python3
"""
Command-line chat interface that works with BOTH:
- Databricks Foundation Models
- OpenAI Models

Set MODE in .env to switch between them
"""
...
if MODE == "openai":
    # OpenAI Models
    AVAILABLE_MODELS = {
        "1": {"name": "GPT-4o", "id": "gpt-4o"},
        "2": {"name": "GPT-4o Mini", "id": "gpt-4o-mini"},
        "3": {"name": "GPT-4 Turbo", "id": "gpt-4-turbo"},
        "4": {"name": "GPT-3.5 Turbo", "id": "gpt-3.5-turbo"},
    }
    SERVICE_NAME = "OpenAI"
else:
    # Databricks Foundation Models
...
def print_banner():
    ...
    print(f"  {SERVICE_NAME} Foundation Models - Chat Interface")
...
def main():
    ...
    if MODE == "openai":
        if not os.environ.get("OPENAI_API_KEY"):
            print(f"{Colors.RED}Error: Missing OpenAI API key!{Colors.END}")
        ...
        try:
            ...
            print(f"{Colors.GREEN}✓ Connected to OpenAI{Colors.END}")
        except Exception as e:
            print(f"{Colors.RED}Error connecting to OpenAI: {e}{Colors.END}")
            sys.exit(1)
    else:
    ...
    while True:
        model_choice = input(f"{Colors.BOLD}Select a model (1-{model_count}) [default: 1]: {Colors.END}").strip()
        if not model_choice:
            model_choice = "1"
        if model_choice in AVAILABLE_MODELS:
            selected_model = AVAILABLE_MODELS[model_choice]
            break
        print(f"{Colors.RED}Invalid choice. Please select 1-{model_count}.{Colors.END}")

    print(f"\n{Colors.GREEN}Selected: {selected_model['name']}{Colors.END}")
    print(f"{Colors.BLUE}Model ID: {selected_model['id']}{Colors.END}")
    print(f"{Colors.BLUE}Service: {SERVICE_NAME}{Colors.END}\n")
    ...
    while True:
        try:
            ...
            if user_input.lower() == 'switch':
                print()
                print_models()
                model_choice = input(f"{Colors.BOLD}Select a model (1-{model_count}): {Colors.END}").strip()
                if model_choice in AVAILABLE_MODELS:
                    selected_model = AVAILABLE_MODELS[model_choice]
